...
flag AS (
    SELECT * FROM {{ ref('int_features_flag_transfo') }}
),

sig AS (
    SELECT * FROM {{ ref('int_features_signaletique') }}
),

aff AS (
    SELECT * FROM {{ ref('int_features_affaire') }}
),

cib AS (
    SELECT * FROM {{ ref('int_features_ciblage') }}
),

sav AS (
    SELECT * FROM {{ ref('int_features_sav') }}
),

recla AS (
    SELECT * FROM {{ ref('int_features_reclamation') }}
),

dem AS (
    SELECT * FROM {{ ref('int_features_demande_info') }}
),

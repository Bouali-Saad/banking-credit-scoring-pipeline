...
SELECT
    ...
    MAX(CASE WHEN UPPER(produit_wfs) LIKE '%RENOUVELABLE%'
             THEN 1 ELSE 0 END)                             AS flag_credit_renouvelable,

    
    COUNT(CASE WHEN UPPER(produit_wfs) LIKE '%EQUIPEMENT%'
               THEN 1 END)                                  AS nb_credits_equip,
    COUNT(CASE WHEN UPPER(produit_wfs) LIKE '%AUTO%'
               THEN 1 END)                                  AS nb_credits_auto,
    COUNT(CASE WHEN UPPER(produit_wfs) LIKE '%RENOUVELABLE%'
               THEN 1 END)                                  AS nb_credits_renouvelable,
    COUNT(CASE WHEN UPPER(produit_wfs) LIKE '%PRET%'
               OR UPPER(produit_wfs) LIKE '%PERSONNEL%'
               THEN 1 END)                                  AS nb_credits_perso,

    
    SUM(CASE WHEN UPPER(produit_wfs) LIKE '%EQUIPEMENT%'
             THEN montant_init_brut ELSE NULL END)          AS sum_mt_equip,
    SUM(CASE WHEN UPPER(produit_wfs) LIKE '%AUTO%'
             THEN montant_init_brut ELSE NULL END)          AS sum_mt_auto,
    SUM(CASE WHEN UPPER(produit_wfs) LIKE '%RENOUVELABLE%'
             THEN montant_init_brut ELSE NULL END)          AS sum_mt_renouvelable,
    SUM(CASE WHEN UPPER(produit_wfs) LIKE '%PRET%'
             OR UPPER(produit_wfs) LIKE '%PERSONNEL%'
             THEN montant_init_brut ELSE NULL END)          AS sum_mt_perso,

    
    AVG(CASE WHEN UPPER(produit_wfs) LIKE '%EQUIPEMENT%'
             THEN montant_init_brut ELSE NULL END)          AS avg_mt_equip,
    AVG(CASE WHEN UPPER(produit_wfs) LIKE '%AUTO%'
             THEN montant_init_brut ELSE NULL END)          AS avg_mt_auto,
    AVG(CASE WHEN UPPER(produit_wfs) LIKE '%RENOUVELABLE%'
             THEN montant_init_brut ELSE NULL END)          AS avg_mt_renouvelable,
    AVG(CASE WHEN UPPER(produit_wfs) LIKE '%PRET%'
             OR UPPER(produit_wfs) LIKE '%PERSONNEL%'
             THEN montant_init_brut ELSE NULL END)          AS avg_mt_perso,
